...
class DataLoader(DataAccess):
    def __init__(self, engine, schema_name, mandatory_srids=None):
        ...
        self.tables = dict((t.name, t) for t in tables)
        self.classes = dict((t.name, self.get_table_class(t.name))
                            for t in tables)
    ...
    def repair_geometry(self, geometry_source):
        ...
        logger.debug("running geometry QC and repair: %s" %
        ...
        self.session.execute(sqlalchemy.update(
            cls.__table__, values={
                geom_attr: sqlalchemy.func.st_multi(
                    sqlalchemy.func.st_buffer(geom_attr, 0))
    ...
    def reproject(self, geometry_source_id, from_column, to_srid):
        ...
        GeometrySource = self.classes['geometry_source']
        TableInfo = self.classes['table_info']
        geometry_source = self.session.query(GeometrySource).filter(
            GeometrySource.id == geometry_source_id).one()
        table_info = self.session.query(TableInfo).filter(
            TableInfo.id == geometry_source.table_info_id).one()
    ...
    def register_table(self, table_name, geom=False, srid=None, gid=None):
        ...
        if geom:
            ...
            if column is None:
                raise Exception(
                    "Cannot automatically determine geometry column for `%s'" % table_name)
        ...
        linkage = GeometryLinkage(
            geometry_source_id=geo_source[0],
            # geometry_source=geo_source,
            geo_column=geo_column,
            table_info_id=attr_table.id,
...
class DataLoaderResult:
    ...
    def dump(self, target_dir):
        ...
        if code != 0:
            ...
            logger.info(
                "load with: pg_restore --username=user --dbname=db /path/to/%s" % self._schema_name)
